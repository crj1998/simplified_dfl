...
class FaceData(Dataset):
    def __init__(self, data_path):
        ...
        self.image_files_src = 2 * self.image_files_src
        self.image_files_dst = 2 * self.image_files_dst
...
def draw_results(reconstruct_src, target_src, reconstruct_dst, target_dst, fake, loss_src, loss_dst):
    ...
    axes.plot(loss_src, label='loss src', color="green")
    axes.plot(loss_dst, label='loss dst', color="blue")
    plt.legend()
    plt.title('Loss')
    canvas = fig.canvas
    canvas.draw()
...
def train(data_path: str, model_name='Quick96', new_model=False, saved_models_dir='saved_model'):
    ...
    while run:
        epoch += 1
        for ii, (warp_im_src, target_im_src, warp_im_dst, target_im_dst) in tqdm(enumerate(dataloader), ncols=100, total=len(dataloader)):
        ...
        cv2.imwrite(f'results.jpg', result_image * 255)
